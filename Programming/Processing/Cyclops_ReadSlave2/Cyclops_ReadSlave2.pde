...
void draw() {
  if(errorFlag==false)
  {
    ...
    // First want to try and get any data which may appear on the serial port
    getData();
    text("Voltage 1: "+Str1, 200, 50); 
    text("Voltage 2: "+Str2, 200, 100); 
    text("Voltage 3: "+Str3, 200, 150); 
    ...
    // Draws a circle depednign upon the voltage
    
    // Scale the mouseX value from 0 to 640 to a range between 0 and 175
    float c = map(Int2, 0, width, 0, 175);
    // Scale the mouseX value from 0 to 640 to a range between 40 and 300
    float d = map(Int2, 0, width, 40, 300);
    fill(255, c, 0);
